...
Включите **Показывать порты** под режимами отображения, когда нужны `узел:порт` или `IP:порт`.
Выбор сохраняется, поиск и все действия копирования учитывают показанный формат.
Разные порты остаются отдельными строками даже при группировке; IPv6 записывается как `[адрес]:порт`.
Порты берутся из URL запросов, включая стандартные, например 443 для HTTPS. Это не измеренные
порты соединений. Старые записи без порта остаются без него. Произвольный TCP/UDP-трафик
недоступен через браузерный API `webRequest`.
